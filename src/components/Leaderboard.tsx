  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        >
          <ArrowLeft className="w-4 h-4" />{t("lb.menu")}
        </Link>
      </div>
      ...
      >
        ...
        <h1 className="font-display text-5xl font-bold gold-text mb-2">{t("lb.title")}</h1>
        <p className="text-ink-soft">{t("lb.subtitle")}</p>
      </motion.div>
      ...
      {!configured ? (
        <div className="text-center p-10 rounded-2xl border border-white/10 bg-white/[0.02]">
          <p className="text-ink-soft">
            {t("lb.notReady")}{" "}
            <br />
            {t("lb.worksNow")} <strong>{t("menu.ai.title")}</strong> · <strong>hot-seat</strong>.
          </p>
        </div>
      ) : loading ? (
        <div className="text-center text-ink-soft py-10">{t("lb.loading")}</div>
      ) : rows.length === 0 ? (
        <div className="text-center p-10 rounded-2xl border border-white/10 bg-white/[0.02]">
          <p className="text-ink-soft mb-2">{t("lb.empty1")}</p>
          <p className="text-sm text-ink-soft/70">{t("lb.empty2")}</p>
        </div>
      ) : (
          ...
          <div className="lg:col-span-2">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden">
              <div className="grid grid-cols-[40px_1fr_80px_60px_60px_60px] gap-3 px-4 py-3 text-xs uppercase tracking-widest text-ink-soft border-b border-white/10">
                <div>#</div>
                <div>{t("lb.col.player")}</div>
                <div className="text-right">{t("lb.col.points")}</div>
                <div className="text-right">{t("lb.col.wins")}</div>
                <div className="text-right">{t("lb.col.games")}</div>
                <div className="text-right">{t("lb.col.winPct")}</div>
              </div>
          ...
          <div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
              <div className="text-xs uppercase tracking-widest text-ink-soft mb-3 flex items-center gap-2">
                <Users className="w-3 h-3" /> {t("lb.cities.title")}
              </div>
                  ...
                  <div key={s.city} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-ink-soft">
                        {t("lb.cities.stats", { players: s.players, wins: s.wins })}
                      </div>
                    </div>
